...
export default function TrainingTab() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await fetch('/api/cv/training/stats', { headers: getAuthHeaders() });
        if (res.ok) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/cv/training/generate-question', {
        method: 'POST',
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/cv/training/evaluate', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
          theme: selectedTheme,
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className="text-sm text-slate-400">{totalSessions > 0 ? `${totalSessions} entraînements` : 'Aucun entraînement'}</p>
          </div>
