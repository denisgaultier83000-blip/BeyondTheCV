          ...
          <div style={{ background: 'var(--bg-secondary)', padding: '1.5rem', borderRadius: '1rem', border: '1px solid var(--border-color)', marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '2rem', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: '250px' }}>
               ...
               <div style={{ width: '100%', height: '10px', display: 'flex', borderRadius: '5px', overflow: 'hidden', marginBottom: '1rem', background: 'var(--border-color)' }}>
                 ...
                 {lowRiskCount > 0 && <div style={{ width: `${(lowRiskCount/totalWithImpact)*100}%`, background: '#10b981', transition: 'width 1s ease-out' }} title={`${lowRiskCount} ${t('flaw_low', 'Faible')}`} />}
               </div>
               <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                 {highRiskCount > 0 && <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 600, color: '#ef4444' }}><div style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#ef4444' }}/> {highRiskCount} {t('flaw_risk_crits', 'Risque(s) Critique(s)')}</span>}
                 {mediumRiskCount > 0 && <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 600, color: '#f59e0b' }}><div style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#f59e0b' }}/> {mediumRiskCount} {t('flaw_risk_points', 'Point(s) de Vigilance')}</span>}
                 {lowRiskCount > 0 && <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: 600, color: '#10b981' }}><div style={{ width: '10px', height: '10px', borderRadius: '50%', background: '#10b981' }}/> {lowRiskCount} {t('flaw_risk_accepts', 'Défaut(s) Acceptable(s)')}</span>}
               </div>
            </div>
          </div>
        )}
              ...
              <div key={idx} style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '1.5rem', border: `1px solid ${item.impact_level ? impact.border : 'var(--border-color)'}`, boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                  <div style={{ background: 'rgba(239, 68, 68, 0.05)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
                    <h4 style={{ margin: '0 0 0.5rem 0', color: 'var(--danger-text)', fontSize: '0.95rem' }}>{t('flaw_traps', '⚠️ Pièges à éviter')}</h4>
                  ...
                  <div style={{ background: 'rgba(34, 197, 94, 0.05)', padding: '1rem', borderRadius: '0.5rem', border: '1px solid rgba(34, 197, 94, 0.2)' }}>
                    <h4 style={{ margin: '0 0 0.5rem 0', color: 'var(--success)', fontSize: '0.95rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <Lightbulb size={16} /> {t('flaw_coach_advice', 'Le Conseil du Coach')}
                    </h4>
