...
def test_generate_docx_document(test_client: TestClient, mock_db, mocker):
    ...
    mock_generate_docx = mocker.patch("services.cv.generate_cv_docx", return_value="/tmp/fake_cv.docx")
    mocker.patch("os.path.basename", return_value="fake_cv.docx")
    
    # Mock de la réponse de l'IA
    mocker.patch("services.cv.optimize_cv_data", return_value={"optimized_data": {"last_name": "Doe"}})
