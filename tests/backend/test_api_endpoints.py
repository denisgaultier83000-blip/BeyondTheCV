...
def test_get_feedbacks_admin(test_client, mock_db):
    ...
    # On simule un retour de dict depuis la base de données
    mock_db["cursor"].fetchall.return_value = [
        {"id": 1, "feature": "cv_generation", "is_positive": True, "comments": "Super", "created_at": "2026-05-08T10:00:00"}
    ]
...
def test_get_profile_fallback_to_users(test_client, mock_db):
    ...
    data = response.json()
    assert "form" in data
    assert data["form"]["email"] == "test@example.com"
